...
composer require xuejd3/socialiteproviders-xianyan_bbs
```
...
- The listener that you add for this provider is `'SocialiteProviders\\XianYanBbs\\XianYanBbsExtendSocialite@handle',`.
...
```php
'xianyan_bbs' => [
    'client_id'     => env('XIANYANBBS_KEY'),
    'client_secret' => env('XIANYANBBS_SECRET'),
    'redirect'      => env('XIANYANBBS_REDIRECT_URI'),
    'guzzle'        => [
        'headers' => ['User-Agent' => 'xuejd3/socialiteproviders-xianyan_bbs'],
    ]
]
```
...
```php
$user = Socialite::driver('xianyan_bbs')->user();
...
return Socialite::with('xianyan_bbs')->setConfig($config)->redirect();
```
